...
class GroqService {
    ...
    private async buildPrompt(userObjectId: mongoose.Types.ObjectId): Promise<string> {
        ...
        ## 분석 시 필독 및 엄수 사항:
        1. **수입(Income) 제외**: '월급' 및 '수입' 데이터는 절대로 지출 이상 징후(anomaly_alert)로 판단하지 마십시오. 이는 자산 증가이며 긍정적인 신호입니다.
        2. **이상치 기준**: '미계획 고액 지출'은 지출 내역 중 '평소 카테고리 평균' 또는 '사용자의 일반적인 소비 규모'를 크게 상회하는 단일 지출 항목에 대해서만 생성하십시오.
        3. **JSON 형식**: 마크다운 코드 블록 없이 순수 JSON 배열만 반환하십시오.
        
        ## 응답 가이드:
        - 수입이 지출보다 훨씬 크다면: "훌륭한 자산 관리입니다! 남은 순수입을 활용한 투자 방안을 제안해 드릴까요?"와 같은 인사이트 포함.
        - 예정 일정(upcomingSchedules) 활용: "다음 주에 '친구 결혼식' 일정이 있네요. 축의금 지출을 대비해 이번 주는 외식을 줄여보는 건 어떨까요?"와 같이 일정과 지출을 결합한 분석 수행.
        `;
    }
