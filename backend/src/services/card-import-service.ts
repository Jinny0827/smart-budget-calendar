...
function guessCategory(merchant: string): string {
    ...
    if (/식당|음식|카페|커피|베이커리|베이글|치킨|피자|버거|국밥|밥|면|짜장|초밥|스시|고기|삼겹|족발|떡볶이|분식|편의점|gs25|cu|세븐/.test(n)) return '식비';
    if (/지하철|버스|택시|주유|주차|고속|ktx|korail|카카오택시|우버|티머니/.test(n)) return '교통';
    if (/병원|의원|약국|치과|한의원|클리닉|의료|건강/.test(n)) return '의료';
    if (/헬스|피트니스|pt|수영|요가|스포츠|운동|짐|gym/.test(n)) return '운동';
    if (/항공|호텔|여행|투어|숙박|펜션|에어비앤비|모텔/.test(n)) return '여행';
    if (/쿠팡|11번가|지마켓|옥션|마켓컬리|배달의민족|배민|요기요|올리브영|다이소|이마트|홈플러스|롯데마트/.test(n)) return '쇼핑';
