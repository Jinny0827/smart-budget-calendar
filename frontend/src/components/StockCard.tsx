import type { UserStock, BatchPrice } from "../types/finance.ts";


interface Props {
    stock: UserStock;
    price?: BatchPrice;
    onRemove: (id: string) => void;
}

export default function StockCard({ stock, price, onRemove }: Props)  {
    const isPortfolio = stock.type === 'portfolio';
    const changePos = (price?.change_pct ?? 0) >= 0;
    
    // 수익률 계산 (보유 종목만)
    const returnPct = isPortfolio && price && stock.avgPrice ?
        ((price.current_price - stock.avgPrice) / stock.avgPrice * 100).toFixed(2) : null;
    const returnPos = returnPct !== null ? parseFloat(returnPct) > 0 : null;

    return (
        <div style={{
            background: '#111827',
            borderRadius: 12,
            padding: '14px 16px',
            display: 'flex',
            flexDirection: 'column',
            gap: 8,
            minWidth: 160,
        }}>
            {/* 종목명 + 삭제 */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <p style={{ color: '#fff', fontWeight: 'bold', fontSize: 14, margin: 0 }}>{stock.corpName}</p>
                    <p style={{ color: '#6b7280', fontSize: 11, margin: 0 }}>{stock.ticker} · {stock.market.toUpperCase()}</p>
                </div>
                <button
                    onClick={() => onRemove(stock._id)}
                    style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 16 }}
                >×</button>
            </div>

            {/* 현재가 */}
            {price ? (
                <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                        <span style={{ color: '#fff', fontSize: 18, fontWeight: 'bold' }}>
                            {stock.currency === 'USD'
                                ? `$${price.current_price.toLocaleString()}`
                                : `${price.current_price.toLocaleString()}원`}
                        </span>
                        <span style={{ color: changePos ? '#22c55e' : '#ef4444', fontSize: 13, fontWeight: 'bold' }}>
                            {changePos ? '▲' : '▼'} {Math.abs(price.change_pct)}%
                        </span>
                    </div>

                    {/* 보유 종목 수익률 */}
                    {isPortfolio && returnPct !== null && (
                        <div style={{
                            background: '#1f2937',
                            borderRadius: 8,
                            padding: '6px 10px',
                            display: 'flex',
                            justifyContent: 'space-between',
                            fontSize: 12,
                        }}>
                            <span style={{ color: '#9ca3af' }}>
                                {stock.quantity?.toLocaleString()}주 · 평균 {stock.avgPrice?.toLocaleString()}
                            </span>
                            <span style={{ color: returnPos ? '#22c55e' : '#ef4444', fontWeight: 'bold' }}>
                                {returnPos ? '+' : ''}{returnPct}%
                            </span>
                        </div>
                    )}
                </>
            ) : (
                <p style={{ color: '#6b7280', fontSize: 13, margin: 0 }}>주가 로딩 중...</p>
            )}

            {/* 타입 뱃지 */}
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <span style={{
                    fontSize: 10,
                    padding: '2px 8px',
                    borderRadius: 99,
                    background: isPortfolio ? '#1d4ed8' : '#374151',
                    color: isPortfolio ? '#bfdbfe' : '#9ca3af',
                }}>
                    {isPortfolio ? '보유' : '관심'}
                </span>
            </div>
        </div>
    );
}